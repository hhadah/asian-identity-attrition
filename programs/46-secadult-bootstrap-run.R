# ================================================================
# Multinomial Logit Bootstrap Runner (Fixed-Covariate Version)
# Only uses: value, Female, EducYears, frac_asian, lnftotval,
#            Age, Age_sq, Age_cube, Age_quad, region_year
# ================================================================
...
# Lock PP/ME variables to the allowed set only
PP_VARS     <- c("value", "Female", "EducYears", "frac_asian", "lnftotval")
GRID_STEP   <- 0.5
ME_VARS     <- c("value", "Female", "EducYears", "frac_asian", "lnftotval")
...
# Load & prepare data
# ---------------------------
CPS_IAT <- readr::read_csv(file.path(git_mdir, "CPS_IAT_asian_adults.csv.zip")) |>
...
# ---------------------------
# Fixed multinomial logit model (single formula everywhere)
# ---------------------------
fit_multinomial_model <- function(data_subset, generation="second", ancestry_filter=NULL){
  fml <- identity_choice ~ value + Female + EducYears + frac_asian + lnftotval +
    Age + Age_sq + Age_cube + Age_quad + region_year
  .log_info(sprintf("[multinom] model fixed; gen=%s | anc=%s",
                    generation, if (is.null(ancestry_filter)) "none" else ancestry_filter))
...
make_var_grid <- function(data_subset, var_name, grid_step=0.5){
  if (var_name %in% c("Female")) {
    return(tibble::tibble(x_val = c(0, 1)))
  }
  rng <- range(data_subset[[var_name]], na.rm = TRUE)
  if (var_name == "frac_asian") {
    rng[1] <- max(0, rng[1]); rng[2] <- min(1, rng[2])
  }
  if (var_name %in% c("lnftotval", "frac_asian")) {
    if (!is.finite(rng[1]) || !is.finite(rng[2])) {
      stop(sprintf("Invalid range for %s", var_name))
    }
    if (abs(rng[2] - rng[1]) < .Machine$double.eps) {
      return(tibble::tibble(x_val = rng[1]))
    }
    grid_vals <- seq(rng[1], rng[2], length.out = 5)
    return(tibble::tibble(x_val = as.numeric(grid_vals)))
  }
  if (var_name == "EducYears") {
    pretty_vals <- pretty(rng, n = 6)
    pretty_vals <- pretty_vals[pretty_vals >= rng[1] & pretty_vals <= rng[2]]
    return(tibble::tibble(x_val = sort(unique(pretty_vals))))
  }
  ...
  tibble::tibble(x_val = sort(unique(c(rng[1], interior, rng[2]))))
}
...
representative_row <- function(data_subset){
  ...
  for (nm in names(data_subset)) {
    if (nm %in% c("asian_identity","weight")) next
...
compute_AME <- function(model, data_subset, var_name, h_frac=0.1){
  ...
  if (var_name %in% c("Female")) {
    ...
    } else if (is.logical(d[[var_name]])) { 
      d0[[var_name]] <- FALSE; d1[[var_name]] <- TRUE
    } else { 
...
boot_pp_stat <- function(data, indices, var_name, grid, generation, ancestry_filter=NULL, progress_env=NULL){
  ...
  as.vector(pred)  # column-major order
}
...
# --- Bootstrap function for multinomial logit models ---
bootstrap_multinom_boot <- function(data_subset, generation="second", ancestry_filter=NULL,
  ...
  for (var_name in pp_vars){
    ...
    if (!is.null(main_model)) {
      ...
      if (is.null(colnames(main_pred))) colnames(main_pred) <- outcome_levels
      ...
      if (sum(valid_rows) > 0){
        ...
        ci_df <- boot_long |>
          ...
          dplyr::mutate(
            ...
            # widen collapsed intervals for plotting
...
# ---------------------------
run_multinom_boot_and_save <- function(data_subset, generation, ancestry_filter, label_slug, label_pretty,
  ...
  if (resume && file.exists(paths$rds) && !overwrite) {
    ...
    .log_info(sprintf("=== Bootstrapping multinomial block: %s (gen=%s | anc=%s) ===",
...
boot_second_aw_adults <- run_multinom_boot_and_save(
  ...
  label_slug  = "second_AW_adults_multinom", label_pretty = "Second gen adults: AW parents (Multinomial)",
  seed = 3029
)

boot_second_wa_adults <- run_multinom_boot_and_save(
  data_subset = CPS_IAT_multinomial |> filter(SecondGen_Asian==1, WA_0bj==1),
  generation  = "second", ancestry_filter = "WA_0bj",
  label_slug  = "second_WA_adults_multinom", label_pretty = "Second gen adults: WA parents (Multinomial)",
  seed = 3030
)
