...
#-------------------------------------------------
# bootstrap function (not used in optimized version)
#-------------------------------------------------
source(file.path(programs,"boot-helper.R"))

# =========================
# PLOTTING WITH BOOTSTRAP CIs
# =========================
...
plot_pp_boot <- function(pp_summ, var_name, gen_label) {
  ...
  ggplot(pp_summ, aes(x = x_val, y = p_main, color = group_labeled, group = group_labeled)) +
    ...
    labs(x = x_label, y = "Probability", subtitle = gen_label,
         caption = "Lines: main-sample predictions. Shaded bands: 95% bootstrap CIs.") +
    theme_customs() +
...
plot_me_boot <- function(me_summ, gen_label) {
  ...
  me_summ |>
    ...
    labs(y = NULL, x = "Average marginal effect (per unit of variable)",
         subtitle = gen_label,
         caption = "Points: main-sample AMEs. Bars: 95% bootstrap CIs across resamples.") +
    theme_customs() +
    theme(legend.position = "bottom")
}

# =========================
# RUN BOOTSTRAP EXAMPLES
# =========================

# Choose which variable to put on the PP x-axis
PP_VAR <- "value"       # Anti-Asian bias index
GRID_STEP <- 0.5        # matches your plot grid
ME_VARS <- c("value","Female","MomGradCollege","DadGradCollege")

cat("Bootstrapping: All generations...\n")
boot_all <- bootstrap_multinom(
  ...
  B             = N_BOOTSTRAP,
  n_cores       = N_CORES,
  stratify_by   = "region_year",  # preserves region-year composition
  seed          = 2024
)
...
# Plots (bootstrap CIs)
plot_pp_all_boot <- plot_pp_boot(boot_all$pp, PP_VAR, "All generations")
plot_me_all_boot <- plot_me_boot(boot_all$me, "All generations")

# Save
ggsave(file.path(figures_wd, "boot_pp_all_generations.png"), plot_pp_all_boot, width = 8, height = 6, dpi = 300)
ggsave(file.path(figures_wd, "boot_me_all_generations.png"), plot_me_all_boot, width = 10, height = 6, dpi = 300)

# Second generation, AW families
boot_second_aw <- bootstrap_multinom(
  data_subset   = CPS_IAT_multinomial |> dplyr::filter(SecondGen_Asian == 1, AW_0bj == 1),
  generation    = "second",
...
plot_pp_second_aw <- plot_pp_boot(boot_second_aw$pp, PP_VAR, "Second gen: AW parents")
