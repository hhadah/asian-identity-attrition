...
reg <- list(
  ...
  "\\specialcell{(2) \\\\ Interethnic}" = feols(Interethnic ~ 1 + Bias + 
                                                  MomGradCollege + DadGradCollege + age_mom + age_pop +
                                                  yrimmig_mom + yrimmig_pop
                                                | region:year, 
                                                data = CPS_IAT |> filter(Asian_Dad == 1), weights = ~hwtfinl, vcov = ~serial),
  "\\specialcell{(3) \\\\ Interethnic}" = feols(Interethnic ~ 1 + Bias + 
                                                    MomGradCollege + DadGradCollege + age_mom + age_pop +
                                                    yrimmig_mom + yrimmig_pop
                                                  | region:year, 
...
regression_tab <- modelsummary(reg, #exponentiate = c(F, T, F, F),
                               ...
                               # gof_omit = 'DF|Deviance|R2|AIC|BIC|Log.Lik.|F|Std.Errors|RMSE',
                               stars= c('***' = 0.01, '**' = 0.05, '*' = 0.1),
                               title = "Relationship Between Bias and Interethnic Marriages \\label{regtab-logit-02}")  |> 
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"),
                latex_options = c("HOLD_position")
  ) |>
  footnote(number = c("\\\\footnotesize{This is the result to estimating (\\\\ref{eq:inter-interethnic}) as a
