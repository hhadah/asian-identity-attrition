...
Asian_IAT <- Asian_IAT[Asian_IAT$MSANo != "",]
...
skin_grouped_bymsa <- Asian_IAT %>% 
  # filter(White == 1) |> 
  group_by(metfips, year_group#, 
           #month
  ) %>% 
...
# save
write_csv(CPS_IAT, file.path(datasets,"CPS_IAT_asian_msa.csv"))
